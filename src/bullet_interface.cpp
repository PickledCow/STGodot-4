...
void BulletInterface::_init() {
	...
	bullet_pool = (Bullet**)memalloc(sizeof(Bullet*) * total_bullets);
	shot_pool = (Bullet**)memalloc(sizeof(Bullet*) * total_shots);
	item_pool = (Item**)memalloc(sizeof(Item*) * total_items);
	particle_pool = (Particle**)memalloc(sizeof(Particle*) * total_particles);

	persistent_bullet_index = (int*)memalloc(sizeof(int) * total_bullets);
	persistent_shot_index = (int*)memalloc(sizeof(int) * total_shots);
	persistent_item_index = (int*)memalloc(sizeof(int) * total_items);
	persistent_particle_index = (int*)memalloc(sizeof(int) * total_particles);

	RID canvas_parent = this->get_canvas_item();
...
bool BulletInterface::_process_bullet(Bullet* bullet, double delta) {
    ...
    if (bullet->process_mode < B1 || true) { // B type is unimplemented currently
        ...
        else bounce_count = _process_bullet_a3_before(bullet, delta);
    }
    if (bullet->spin) {
...
int BulletInterface::_process_bullet_a3_before(Bullet* bullet, double delta) {
...
PackedInt64Array BulletInterface::create_bullet_a1(Vector2 pos, double speed, double angle, PackedFloat64Array bullet_data, bool glow) {
	if(available_bullets > 0) {
		...
		rendering_server->canvas_item_add_texture_rect(bullet->item_rid, texture_rect, bullets_texture_rid);
		...
		// A1 type settings
		Transform2D xform = Transform2D(0.0, Vector2(0.0, 0.0)).scaled(bullet_data[4] * Vector2(1.0, 1.0)).rotated(angle + bullet_rotation_offset);
		...
		compressed_data.r = bullet_data[1] + bullet_data[0] / bullets_texture_width;
		compressed_data.g = bullet_data[3] + bullet_data[2] / bullets_texture_width;
